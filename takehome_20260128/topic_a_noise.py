...
if __name__ == "__main__":
    ...
    # Plot: show student aux-only and cross-model aux-only for each condition, with CI
    ...
    ax.bar(
        x - w / 2,
        res["student_aux_only_mean"].values,
        yerr=res["student_aux_only_ci95"].values,
        capsize=4,
        label="Student (aux only, shared init)",
    )
    ax.bar(
        x + w / 2,
        res["crossmodel_aux_only_mean"].values,
        yerr=res["crossmodel_aux_only_ci95"].values,
        capsize=4,
        alpha=0.55,
        label="Cross-model (aux only, mismatched init)",
    )

    ax.axhline(res["reference_mean"].iloc[0], ls=":", c="black", label="Reference mean")
    ax.axhline(res["teacher_mean"].iloc[0], ls="--", c="black", label="Teacher mean")

    ax.set_xticks(x)
    ax.set_xticklabels(res["condition"].tolist(), rotation=35, ha="right")
    ax.set_ylabel("MNIST test accuracy (mean ± 95% CI across N_MODELS)")
    ax.set_title("Experiment 1: Distillation input distribution and scale")
    ax.yaxis.grid(True, alpha=0.3)
    ax.legend(fontsize=9)

    plt.tight_layout()
    fig_path = f"plots_a/{script_name}_noise_sweep.png"
